...
```{r}
total_counts %>% summarize(total_count = n()) 

year_counts %>% summarize(year_count = n())
...
year_and_totals <- year_counts %>% 
  left_join(total_counts, by = c("year"))

year_and_totals <- year_and_totals %>% 
...
year_and_totals %>% 
  filter(year >= 1850, term == '1883' | term == '1910' | term =='1950') %>% 
  mutate(term_factor = factor(term, levels = c('1883','1910','1950'))) %>% 
  ggplot(aes(x = year, y = proportion, color = term_factor)) +
  geom_line() +
  scale_y_continuous(lab = percent) +
  scale_x_continuous()+
  labs(x = "Year", y = "Proportion", title = "Forgeting the Past")

```

## Your written answer

ANSWER: Yes, the version 2 of the NGram data produces a similar results to those presented in the paper by Michel et al. (2011). However, there results are based on an earlier version of the data.
...
Similarities:
*Same trend
*Same looking half-life

Differences:
*Smoother lines on Ngram Viwer
*Slightly higher peaks percentage on my plot. 
...
year_and_totals %>% 
  filter(year >= 1850, term == '1883' | term == '1910' | term == '1950') %>%
  mutate(terms = factor(term, levels = c('1883','1910','1950'))) %>%
  ggplot(aes(x = year, y = volume, color = terms)) +
  geom_line() + 
  scale_y_continuous(lab = comma) +
  labs(x = "Year", y = "Raw Count")
...
I belive that the diferrece is directly related to the diference in datasets. The results of Michel et al. were based on the previous version of the Ngram. Since, Ngram keeps being updated the raw caount is countiniously growing over time.
...
Note that Michel et al. (2011) do something more complicated to estimate the half-life—see section III.6 of the Supporting Online Information—but they claim that both approaches produce similar results. Does version 2 of the NGram data produce similar results to those presented in Michel et al. (2011), which are based on version 1 data? (Hint: Don’t be surprised if it doesn’t.)
...
peaks <- year_and_totals %>% 
  filter(year >= 1850) %>%  #, term == '1883'|term == '1910'|term == '1950') %>% 
  group_by(term) %>% 
  filter(proportion == max(proportion)) %>%
  rename(peak_year = year, peak_prop = proportion) %>%
  ungroup() #%>% View()
# View(peaks)
# View(year_and_totals)
df1 <- year_and_totals %>% left_join(peaks, by = c("term")) #%>% view
 View(df1)
#============================================================================= 
# peaks <- year_and_totals %>% 
#   filter(year >= 1850 ) %>% #, term == '1883'|term == '1910'|term == '1950') %>% 
#   group_by(term) %>%
#   filter(proportion == max(proportion)) %>%
#   mutate(peak_year = year, peak_prop = max(proportion))
# 
# df1 <- year_and_totals %>% left_join(peaks, by = c("term")) # %>% view 
...
half_life <- df1 %>% 
  ...
  group_by(peak_year) %>% 
  mutate(year_min = min(year) - peak_year) %>% 
  ungroup() #%>% View()
#View(half_life)
